...
function vimarActualizarCapaModalBody() {
    ...
    const hist = document.getElementById('vimar-modal-historial-detalle')?.classList.contains('vimar-modal-confirmar--visible');
    document.body.classList.toggle('vimar-modal-abierto', !!(c || a || p || f || prod || info || cat || ecat || hist));
}
...
function vimarModalGlobalKeydown(e) {
    ...
    const checks = [
        ...
        { id: 'vimar-modal-historial-detalle', fn: () => cerrarModalHistorialDetalle() },
    ];
...
// --- Modal Detalle de Historial (ventas/gastos/ingresos) ---

let _vimarHistorialModalKey = null;

function abrirModalHistorialDetalle(key, titulo, totalHtml, contenidoHtml) {
    const root = document.getElementById('vimar-modal-historial-detalle');
    if (!root) return;
    _vimarHistorialModalKey = key;
    document.getElementById('vimar-modal-historial-titulo').textContent = titulo || '';
    document.getElementById('vimar-modal-historial-total').innerHTML = totalHtml || '';
    document.getElementById('vimar-modal-historial-contenido').innerHTML = contenidoHtml || '';
    root.setAttribute('aria-hidden', 'false');
    root.classList.add('vimar-modal-confirmar--visible');
    vimarActualizarCapaModalBody();
    inicializarModalesVimarTeclado();
...
// Refresca el contenido del modal solo si ya está abierto mostrando esta misma "key"
// (se usa tras editar/eliminar algo dentro, para no dejar datos desactualizados)
function actualizarModalHistorialSiAbierto(key, titulo, totalHtml, contenidoHtml) {
    const root = document.getElementById('vimar-modal-historial-detalle');
    if (!root || _vimarHistorialModalKey !== key) return;
    if (!root.classList.contains('vimar-modal-confirmar--visible')) return;
    document.getElementById('vimar-modal-historial-titulo').textContent = titulo || '';
    document.getElementById('vimar-modal-historial-total').innerHTML = totalHtml || '';
    document.getElementById('vimar-modal-historial-contenido').innerHTML = contenidoHtml || '';
}

function cerrarModalHistorialDetalle(conSonido = true) {
    const root = document.getElementById('vimar-modal-historial-detalle');
    if (!root) return;
    const estabaAbierto = root.classList.contains('vimar-modal-confirmar--visible');
    root.setAttribute('aria-hidden', 'true');
    root.classList.remove('vimar-modal-confirmar--visible');
    _vimarHistorialModalKey = null;
    vimarActualizarCapaModalBody();
