...
function vimarActualizarBarraAtajos() {
    ...
    // Botones de acción con atajo dinámico (reflejan la config actual del usuario)
    const botonesAccion = [
        { id: 'btnGuardarVenta',  atajo: 'venta',  icono: '💾', texto: 'Guardar Venta' },
        { id: 'btnCerrarDia',     atajo: 'dia',    icono: '✅', texto: 'Cerrar Día' },
        { id: 'btnGuardarSemana', atajo: 'semana', icono: '✅', texto: 'Guardar Semana' },
        { id: 'btnExcelCompleto', atajo: 'excel',  icono: '📥', texto: 'Reporte Excel completo' },
    ];
    botonesAccion.forEach(b => {
        const el = document.getElementById(b.id);
        const combo = _atajos[b.atajo] || VIMAR_ATAJOS_DEFAULT[b.atajo];
        if (el) el.textContent = `${b.icono} ${b.texto} (${_comboStr(combo)})`;
    });
}
    ...
    panel.innerHTML = `
        ...
        <div id="vimar-ajustes-caja">
            ...
            <div id="vimar-ajustes-body">
                ...
                <div class="vimar-ajustes-seccion-titulo admin-only" style="margin-top:22px;">Caja</div>
                <div class="vimar-aj-fila admin-only" style="justify-content:space-between;">
                    <span class="vimar-aj-label"><i class="fas fa-coins" style="margin-right:6px;"></i>Monto en caja</span>
                    <input type="number" id="inputCajaReal" class="ticket-input" placeholder="0"
                        oninput="guardarCaja(); calcularTodo();" style="width:140px;flex:none;">
                </div>
