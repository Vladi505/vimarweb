...
async function generarCatalogoPDF() {
    ...
    for (const [cat, data] of Object.entries(categorias)) {
        ...
        if (isArtCat) {
            head      = [['ID', 'PRESENTACIÓN', 'PRECIO UNITARIO', 'PAQUETE']];
            body      = validos.map(p => [obtenerIdProducto(p) || '—', p, `$${baseArticulos[p]||0}`, promoStr(p)||'—']);
            colStyles = {
                0: { cellWidth: TW * 0.13, halign: 'center', fontSize: 7 },
                1: { cellWidth: TW * 0.47, halign: 'center' },
                2: { cellWidth: TW * 0.22, halign: 'center' },
                3: { cellWidth: TW * 0.18, halign: 'center' },
            };
        } else {
            ...
            const wID    = TW * 0.12;
            const wProd  = nL >= 5 ? TW*0.26 : nL >= 4 ? TW*0.30 : TW*0.34;
            const wCol   = (TW - wID - wProd) / nL;
            head         = [['ID', 'PRESENTACIÓN', ...litsUsados.map(l => LT_LABEL[l])]];
            body         = validos.map(p => {
                const lp = preciosLiquidos[p]||{};
                return [obtenerIdProducto(p) || '—', p, ...litsUsados.map(l => lp[l]>0 ? `$${lp[l]}` : '—')];
            });
            colStyles    = {
                0: { cellWidth: wID,   halign: 'center', fontSize: 7 },
                1: { cellWidth: wProd, halign: 'center' },
            };
            litsUsados.forEach((_, i) => { colStyles[i+2] = { cellWidth: wCol, halign: 'center' }; });
        }
    ...
    bloques.forEach((blk) => {
        ...
        doc.autoTable({
            ...
            didParseCell(d) {
                ...
                if (d.column.index === 0 || d.column.index === 1) return; // ID y nombre: color ya definido en bodyStyles
                ...
                } else if (blk.isArt && d.column.index === 3 && v !== '—') {
