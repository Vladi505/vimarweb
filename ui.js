...
function recalcularFila(input) {
    ...
    if (isNaN(cant) || cant <= 0) { 
        ...
        if (preciosLiquidos[nombre]) {
            ...
            // Mayoreo: cantidad > 3 y precio por litro (lp[3]/3) es número cerrado (sin centavos)
            else if (cant > 3 && lp[3]) {
                const puMayoreo = lp[3] / 3;
                if (Number.isInteger(puMayoreo)) {
                    precioFinal = puMayoreo * cant;
                }
            }
...
function filtrarCatalogo() {
    const queryOriginal = document.getElementById('inputBusqueda').value.trim();
    const query = queryOriginal.toLowerCase();

    // Coincidencias por ID (parcial, sin sensibilidad a mayúsculas)
    const productosPorId = queryOriginal ? buscarProductosPorIdParcial(queryOriginal) : [];
    const nombresPorIdLower = new Set(productosPorId.map(n => n.toLowerCase()));
    ...
    contenedores.forEach(contenedor => {
        ...
        cards.forEach(card => {
            ...
            const mostrar = coincideCat || nombreProd.includes(query) || nombresPorIdLower.has(nombreProd);
            if (mostrar) {
...
function renderizarTablasHistorial() {
    ...
    if (registrosPendientes.length > 0) {
        ...
        const cuerpoTemp = document.createElement('div');
        cuerpoTemp.className = 'vimar-scroll-ventas';
        headerTemporal.addEventListener('click', (e) => {
            if (e.target.closest('button')) return;
            abrirModalHistorialDetalle('__temporal__', 'Días sin archivar', `$${totalTemporal.toFixed(2)}`, cuerpoTemp.innerHTML);
        });
        ...
        actualizarModalHistorialSiAbierto('__temporal__', 'Días sin archivar', `$${totalTemporal.toFixed(2)}`, cuerpoTemp.innerHTML);
    ...
    semanas.reverse().forEach((nomSem, semIdx) => {
        ...
        cuerpo.className = 'vimar-scroll-ventas';
        headerSem.addEventListener('click', (e) => {
            if (e.target.closest('button')) return;
            abrirModalHistorialDetalle('sem:' + nomSem, nomSem, `$${totalSemana.toFixed(2)}`, cuerpo.innerHTML);
        });
        ...
        actualizarModalHistorialSiAbierto('sem:' + nomSem, nomSem, `$${totalSemana.toFixed(2)}`, cuerpo.innerHTML);
...
function renderizarHistorialGastos() {
    ...
    semanas.reverse().forEach(nomSem => {
        ...
        const contenidoHtml = `
            <div class="contenido-historial-gasto-inner">
        ...
        headerGasto.addEventListener('click', (e) => {
            if (e.target.closest('button')) return;
            abrirModalHistorialDetalle('gasto:' + nomSem, nomSem, `$${total.toFixed(2)}`, contenidoHtml);
        });
        actualizarModalHistorialSiAbierto('gasto:' + nomSem, nomSem, `$${total.toFixed(2)}`, contenidoHtml);
    });
}
...
function renderizarHistorialIngresosManuales() {
    ...
    semanas.reverse().forEach(nomSem => {
        ...
        headerIngreso.addEventListener('click', (e) => {
            if (e.target.closest('button')) return;
            abrirModalHistorialDetalle('ingreso:' + nomSem, nomSem, `$${total.toFixed(2)}`, contenidoHtml);
        });
        actualizarModalHistorialSiAbierto('ingreso:' + nomSem, nomSem, `$${total.toFixed(2)}`, contenidoHtml);
    });
}

function actualizarGraficas() {
    if (typeof actualizarGraficaPrincipal === 'function') actualizarGraficaPrincipal();
}
...
function renderizarSeccionStocks() {
    ...
    for (const [cat, data] of Object.entries(categorias)) {
        ...
        // Solo omitir cuando el filtro de bajo stock está activo y no hay productos que mostrar.
        // Sin el filtro, mostrar la categoría aunque esté vacía.
        if (soloMostrarBajoStock && productosAMostrar.length === 0) continue;
        ...
        divCat.innerHTML = `
            <div class="stock-cat-header">
                ...
                <button type="button" class="btn-edit-cat-header admin-only" title="Editar categoría" data-cat="${cat}">
                    <i class="fas fa-pencil-alt"></i>
                </button>
            </div>
        `;
        divCat.querySelector('.btn-edit-cat-header')?.addEventListener('click', () => abrirModalEditarCategoria(cat));
...
document.addEventListener('keydown', (e) => {
    ...
    // Navegación con teclado en el catálogo (flechas + Enter) — siempre activa
...
function vimarInicializarApp() {
    ...
    } else if (seccionGuardada === 'graficasMetricas') {
        vimarActivarSeccion('seccionGraficasMetricas');
        setTimeout(() => { if (typeof renderizarGraficasMetricas === 'function') renderizarGraficasMetricas(); }, 100);
    } else {
        vimarActivarSeccionPrincipal();
    }
    ...
    // Actualizar barra de atajos con teclas configuradas
    if (typeof vimarActualizarBarraAtajos === 'function') vimarActualizarBarraAtajos();
}
