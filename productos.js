...
function _poblarSelectCopiarPrecios(excluir) {
    const sel = document.getElementById('mp-copiar-de');
    if (!sel) return;
    sel.innerHTML = '<option value="">— Seleccionar producto —</option>';
    Object.entries(categorias).forEach(([cat, data]) => {
        const prods = [...data.prods].filter(p => p !== excluir).sort();
...
function copiarPreciosDeProducto() {
    const sel = document.getElementById('mp-copiar-de');
    const origen = sel?.value;
    if (!origen) return;
    if (typeof playSound === 'function') playSound('click');

    const catOrigen = _obtenerCategoriaProducto(origen);
    const selCat = document.getElementById('mp-categoria');
    if (selCat && catOrigen && catOrigen !== 'Sin categoría') {
        selCat.value = catOrigen;
        _actualizarSeccionesFormProducto();
    }

    if (catOrigen === 'Artículos') {
        document.getElementById('mp-pu').value = baseArticulos[origen] || '';
        const promo = promosArticulos[origen] || {};
        document.getElementById('mp-promo').value = Object.entries(promo).map(([k, v]) => `${k}:${v}`).join(', ');
    } else {
        const lp = preciosLiquidos[origen] || {};
...
function _rellenarFormProducto(nombre, catNombre) {
    ...
    _poblarSelectCopiarPrecios(nombre);
